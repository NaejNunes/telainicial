...
function preload() {
    game.load.image('sky', 'assets/fundofase6.jpg');
	game.load.image('ground', 'assets/blocofloresta.png');
	game.load.image('blocks', 'assets/blocofloresta1.png');
...
function create() {
	...
	platforms2 = platforms.create(200, 180, 'blocks');
    ...
    for (var i = 0; i < 18; i++)
    {
        var star = stars.create(i * 45, 0, 'star');
...
function update() {
	...
	if(platforms2.position.x <= 600){ platforms2.body.velocity.x = 50;}else{platforms2.position.x = 0;}
...
function collectStar (player, star) {
	...
	if(score < 180)
	{
		...
		window.location.replace("level7.html");
	}
